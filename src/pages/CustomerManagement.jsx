...
const CustomerManagement = () => {
  ...
  const fetchStats = async () => {
    try {
      const response = await fetch("http://localhost:5000/api/admin/customers/stats", {
        headers: {
          Authorization: `Bearer ${getAccessToken()}`,
        },
      });
  ...
  const fetchMonthlyData = async () => {
    try {
      const response = await fetch("http://localhost:5000/api/admin/customers/monthly-data", {
        headers: {
          Authorization: `Bearer ${getAccessToken()}`,
        },
      });
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        ...
        <div className="bg-white rounded-xl shadow p-5">
          ...
          <p className={`text-sm mt-1 ${parseFloat(stats.percentageChange) >= 0 ? 'text-green-500' : 'text-red-500'}`}>
            {parseFloat(stats.percentageChange) >= 0 ? '▲' : '▼'} {Math.abs(parseFloat(stats.percentageChange))}% Last 7 days
          </p>
        </div>
      ...
      <div className="bg-white rounded-xl shadow p-6 mb-6">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-lg font-semibold">Monthly Customer Registrations</h3>
        </div>
              ...
              {currentCustomers.length > 0 ? (
                currentCustomers.map((customer) => (
                  <tr key={customer.customer_id} className="border-b hover:bg-gray-50">
                    ...
                    <td className="p-3">₹{formatCurrency(customer.total_spend)}</td>
                    <td className={`p-3 font-semibold ${getStatusColor(customer.status)}`}>
                      {customer.status}
                    </td>
