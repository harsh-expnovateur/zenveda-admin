...
const TeaManagement = () => {
  ...
  const fetchAllTeas = async () => {
    try {
      setLoading(true);
      const response = await fetch("http://localhost:5000/api/tea");
  ...
  const handleTeaImageUpload = (e) => {
    ...
    setPreviews((prev) => ({
      ...prev,
      teaImages: [...prev.teaImages, ...files.map((f) => URL.createObjectURL(f))],
    }));
  };
  ...
  const removeTeaImage = (index) => {
    setForm((prev) => {
      ...
      const newMainIndex = prev.mainImageIndex === index ? 0 : 
  ...
  const handleSubmit = async (e) => {
    ...
    const loadingToast = toast.loading(editingTeaId ? "Updating tea..." : "Saving tea...");
    
    try {
      const formData = new FormData();

      const ritualsWithText = form.brewingRitual.filter(r => r.text && r.text.trim() !== "");
      ...
      const url = editingTeaId 
        ? `http://localhost:5000/api/tea/${editingTeaId}`
        : "http://localhost:5000/api/tea";
      
      const method = editingTeaId ? "PUT" : "POST";
      ...
      if (response.ok) {
        toast.success(editingTeaId ? "Tea updated successfully!" : "Tea saved successfully!", {
          id: loadingToast,
        });
  ...
  const handleEdit = async (teaId) => {
    try {
      const response = await fetch(`http://localhost:5000/api/tea/${teaId}`);
      const data = await response.json();
      ...
      if (data.success) {
        ...
        setForm({
          ...
          price: tea.packages?.reduce((acc, p) => {
            acc[p.package_name.replace(/\s+/g, "_")] = p.selling_price;
            return acc;
          }, {}) || {},
          sections: tea.sections?.length > 0 ? tea.sections : [{ title: "", content: "" }],
          brewingRitual: tea.rituals?.length > 0 ? tea.rituals.map(r => ({ text: r.step_text, icon: null })) : [{ text: "", icon: null }],
          teaImages: [],
          mainImageIndex: tea.images?.findIndex(img => img.is_main_image) || 0,
          active: tea.is_active,
          selectedIngredients: tea.ingredients?.map(ing => ing.id) || [],
        });
        
        setPreviews({
          teaImages: tea.images?.map(img => `http://localhost:5000/${img.image_url}`) || [],
          brewing: tea.rituals?.map(r => r.icon_url ? `http://localhost:5000/${r.icon_url}` : null) || [],
        });
  ...
  const handleDelete = async (teaId) => {
    if (window.confirm("Are you sure you want to delete this tea? This action cannot be undone.")) {
  ...
  const toggleActive = async (teaId, currentStatus) => {
    try {
      const response = await fetch(`http://localhost:5000/api/tea/${teaId}/toggle`, {
        method: "PATCH",
        headers: {
      ...
      if (response.ok) {
        toast.success(`Tea ${!currentStatus ? 'activated' : 'deactivated'} successfully`);
        await fetchAllTeas();
      } else {
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4">
            <div>
              <h1 className="text-4xl font-bold text-[#1f513f] mb-2">Tea Collection</h1>
            ...
            <div className="bg-white rounded-xl p-5 shadow-sm border border-gray-100">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-3xl font-bold text-[#1f513f]">{teas.length}</p>
                </div>
                <div className="bg-green-100 p-3 rounded-full">
                  <svg className="w-6 h-6 text-[#1f513f]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="bg-white rounded-xl p-5 shadow-sm border border-gray-100">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-3xl font-bold text-green-600">{teas.filter(t => t.is_active).length}</p>
                </div>
                <div className="bg-green-100 p-3 rounded-full">
            ...
            <div className="bg-white rounded-xl p-5 shadow-sm border border-gray-100">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-3xl font-bold text-gray-400">{teas.filter(t => !t.is_active).length}</p>
                </div>
                  ...
                  <tr>
                    <td colSpan="5" className="text-center p-12">
                      <div className="flex flex-col items-center justify-center">
                        ...
                        <p className="text-gray-500 font-medium">No teas added yet</p>
                        <p className="text-gray-400 text-sm mt-1">Click "Add New Tea" to get started</p>
                      </div>
                    </td>
                  </tr>
                ) : (
                  teas.map((tea) => (
                    <tr key={tea.id} className="hover:bg-green-50/50 transition-colors">
                      <td className="p-4">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="font-semibold text-gray-800">{tea.name}</span>
                        </div>
                      </td>
                      <td className="p-4">
                        <span className="text-gray-500 text-sm font-mono bg-gray-100 px-2 py-1 rounded">{tea.slug || "-"}</span>
                      </td>
                      <td className="p-4">
                        {tea.tag ? (
              ...
              <div className="sticky top-0 bg-gradient-to-r from-[#1f513f] to-[#2d7356] text-white p-6 rounded-t-3xl z-10 shadow-md">
                <div className="flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-green-100 text-sm">Fill in the details below to manage your tea collection</p>
                  </div>
                  <button
                    onClick={() => {
                      setShowModal(false);
                ...
                <form onSubmit={handleSubmit} className="space-y-8">
                  ...
                  <div className="bg-gradient-to-br from-green-50 to-blue-50 rounded-2xl p-6 border border-green-100">
                    ...
                    <div className="grid md:grid-cols-2 gap-6 mt-6">
                      <div>
                        <label className="block font-semibold text-gray-700 mb-2">
                          Tag <span className="text-xs text-gray-500">(e.g., BEST SELLER)</span>
                        </label>
                        <input
                          name="tag"
                          value={form.tag}
                  ...
                  <div className="bg-gradient-to-br from-amber-50 to-orange-50 rounded-2xl p-6 border border-amber-100">
                    ...
                    <div className="flex flex-wrap gap-3 mb-4">
                      {["100gm", "200gm", "250gm", "500gm", "1000gm"].map((pkg) => (
                        <label
                          key={pkg}
                        ...
                        >
                          <input
                            type="checkbox"
                            checked={form.selectedPackages?.includes(pkg) || false}
                            onChange={(e) => {
                    ...
                    {form.selectedPackages?.length > 0 && (
                      <div className="mt-6">
                        <h4 className="font-semibold text-gray-700 mb-3">Set Prices</h4>
                        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                          {form.selectedPackages.map((pkg) => {
                            const pkgKey = pkg.replace(/\s+/g, "_");
                            return (
                              <div key={pkg} className="relative">
                                <label className="block text-sm font-medium text-gray-600 mb-1">{pkg}</label>
                  ...
                  <div className="bg-gradient-to-br from-purple-50 to-pink-50 rounded-2xl p-6 border border-purple-100">
                    ...
                    <div className="border-2 border-gray-200 rounded-xl p-4 bg-white max-h-60 overflow-y-auto">
                      {ingredients.length === 0 ? (
                        <p className="text-gray-500 text-center py-4">No ingredients available. Please add ingredients first.</p>
                      ) : (
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
                          {ingredients.map((ingredient) => (
                            ...
                            >
                              <input
                                type="checkbox"
                                checked={form.selectedIngredients.includes(ingredient.id)}
                    ...
                    {form.selectedIngredients.length > 0 && (
                      <p className="text-sm text-gray-600 mt-3 bg-white px-3 py-2 rounded-lg border border-gray-200">
                        <span className="font-semibold">{form.selectedIngredients.length}</span> ingredient(s) selected
                      </p>
                    )}
                  </div>
                  ...
                  <div className="bg-gradient-to-br from-blue-50 to-cyan-50 rounded-2xl p-6 border border-blue-100">
                    ...
                    <p className="text-sm text-gray-600 mb-4">Add sections like Health Benefits, Tasting Notes, etc.</p>
                    
                    <div className="space-y-4">
                      {form.sections.map((sec, i) => (
                        <div key={i} className="bg-white border-2 border-gray-200 p-5 rounded-xl relative">
                  ...
                  <div className="bg-gradient-to-br from-yellow-50 to-amber-50 rounded-2xl p-6 border border-yellow-100">
                    ...
                    <div className="space-y-4">
                      {form.brewingRitual.map((step, i) => (
                        <div key={i} className="bg-white border-2 border-gray-200 p-5 rounded-xl">
                          <div className="flex items-start gap-4 mb-3">
                            ...
                            <div className="flex-1">
                              <input
                                ...
                                value={step.text}
                                onChange={(e) => handleBrewingChange(i, "text", e.target.value)}
                          ...
                          <div className="pl-12">
                            <label className="block text-sm font-medium text-gray-600 mb-2">Step Icon (Optional)</label>
                            <input
                              type="file"
                              accept="image/*"
                              onChange={(e) => handleBrewingIconUpload(i, e.target.files[0])}
                    ...
                    <div className="mb-4">
                      <label className="block w-full cursor-pointer">
                        <div className="border-2 border-dashed border-gray-300 rounded-xl p-8 text-center hover:border-[#1f513f] hover:bg-white transition-all bg-white">
                          ...
                          <p className="text-gray-600 font-medium mb-1">Click to upload tea images</p>
                    ...
                    {previews.teaImages.length > 0 && (
                      <>
                        ...
                        <p className="text-sm text-gray-600 mt-4 bg-white px-4 py-2 rounded-lg border border-gray-200">
                          <span className="font-semibold">Tip:</span> Main image appears first on the website. Hover over images to change or remove them.
                        </p>
                      </>
                    )}
                  </div>
                  ...
                  <div className="bg-gradient-to-br from-gray-50 to-slate-50 rounded-2xl p-6 border border-gray-200">
                    <label className="flex items-center gap-3 cursor-pointer group">
                      ...
                      <div>
                        <span className="font-semibold text-gray-800 block">Active Status</span>
                        <span className="text-sm text-gray-500">Make this tea visible on the website</span>
                      </div>
                    </label>
                  </div>
                </form>
